...
let literal_color = function "true" | "false" -> Green | _ -> BrightGreen

let type_color = Magenta

let new_type_color = BrightCyan

let keyword_color = Blue

let symbol_color = Magenta

let variable_color = Cyan

let punctuation_color = Default
...
and unparse_template_decl ?(indent = "") ?(abbreviated = true)
  ...
  Buffer.add_string buffer @@ indent ;
  Buffer.add_string buffer @@ colorize ~color:keyword_color "tmpl " ;
  Buffer.add_string buffer @@ colorize ~color:variable_color id.data ;
  Buffer.add_string buffer @@ "( " ;
  unparse_list ~buffer ~separator:", "
  ...
  unparse_list ~buffer
    ~initial:(colorize ~color:symbol_color " => ")
    ~separator:", "
    (fun ~buffer ex ->
      Buffer.add_string buffer @@ colorize ~color:variable_color ex.data )
    export
...
and unparse_event ?(indent = "") ?(abbreviated = true) ?(print_value = false)
  ...
  let unparse_io ?(indent = "") ?(buffer = Buffer.create 100) io =
    ...
    | Input ty ->
        ...
        Buffer.add_string buffer @@ "[" ;
        Buffer.add_string buffer @@ colorize ~color:keyword_color "?" ;
...
and unparse_template_inst ?(indent = "") ?(buffer = Buffer.create 100) inst =
  ...
  unparse_list ~buffer
    ...
    ~separator:", "
    (fun ~buffer x ->
      Buffer.add_string buffer @@ colorize ~color:variable_color x.data )
    x ;
  Buffer.contents buffer
...
and unparse_relation ?(indent = "") ?(abbreviated = true)
  ...
  let control_relation_color = function
    | Response -> Blue
    | Condition -> Yellow
    | Include -> Green
    | Exclude -> Red
  ...
  let unparse_relation_type ?(indent = "") ?(buffer = Buffer.create 100) ~guard
      relation_type =
    Buffer.add_string buffer @@ indent ;
    let color = control_relation_color relation_type in
    let arrow_start =
...
and unparse_annotation ?(indent = "") ?(abbreviated = true)
  ...
  | IfElse {condition; then_branch; else_branch} ->
      Buffer.add_string buffer @@ colorize ~color:keyword_color "if " ;
      ...
      | None -> ()
      | Some branch ->
          Buffer.add_string buffer @@ colorize ~color:keyword_color "else:\n" ;
          unparse_subprogram ~indent:(indent ^ "  ") ~buffer branch |> ignore ) ;
      (* Buffer.add_string buffer @@ indent ; *)
      Buffer.add_string buffer @@ colorize ~color:keyword_color "/if" ;
      Buffer.add_string buffer @@ "\n"
  | Foreach (id, expr, body) ->
      Buffer.add_string buffer @@ colorize ~color:keyword_color "foreach " ;
      Buffer.add_string buffer @@ colorize ~color:variable_color id.data ;
      Buffer.add_string buffer @@ colorize ~color:keyword_color " in " ;
...
and unparse_ty ?(indent = "") ?(buffer = Buffer.create 100) ty =
  ( match ty with
  | UnitTy -> Buffer.add_string buffer @@ colorize ~color:type_color "Unit"
  ...
  | RecordTy fields ->
      ...
      unparse_list ~buffer ~separator:", "
        (fun ~buffer (field, ty) ->
          Buffer.add_string buffer @@ colorize ~color:variable_color field.data ;
          Buffer.add_string buffer @@ ": " ;
          unparse_ty ~indent ~buffer ty.data |> ignore )
        fields ;
      Buffer.add_string buffer @@ " }"
  | ListTy ty ->
      Buffer.add_string buffer @@ colorize ~color:type_color "List" ;
      Buffer.add_string buffer @@ "[" ;
...
and unparse_expr ?(indent = "") ?(buffer = Buffer.create 100) expr =
  ...
  let unparse_unary_op ?(buffer = Buffer.create 100) op =
    let str = match op with Negation -> "!" | Minus -> "-" in
    Buffer.add_string buffer @@ str
  in
  ...
  | StringLit s ->
      (* Buffer.add_string buffer @@ "\"" ;
         ...
         Buffer.add_string buffer @@ "\"" *)
      Buffer.add_string buffer
      @@ colorize ~color:(literal_color ("\"" ^ s ^ "\"")) ("\"" ^ s ^ "\"")
  | Parenthesized e ->
      Buffer.add_string buffer @@ "(" ;
  ...
  | Identifier id ->
      Buffer.add_string buffer @@ colorize ~color:variable_color id.data
  | Trigger ->
      Buffer.add_string buffer @@ colorize ~color:keyword_color trigger_id
  | PropDeref (e, prop) ->
      unparse_expr ~indent ~buffer e |> ignore ;
      Buffer.add_string buffer @@ "." ;
      Buffer.add_string buffer @@ colorize ~color:variable_color prop.data
  | List es ->
      Buffer.add_string buffer @@ "[ " ;
      unparse_list ~buffer ~separator:", "
      ...
      Buffer.add_string buffer @@ " ]"
  | Range (s, e) ->
      Buffer.add_string buffer @@ colorize ~color:keyword_color "@range" ;
      Buffer.add_string buffer @@ "(" ;
