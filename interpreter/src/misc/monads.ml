module ResultMonad = struct
  ...
  let bind (x : ('a, 'b) t) (f : 'a -> ('c, 'b) t) : ('c, 'b) t = match x with Ok x -> f x | Error e -> Error e

  let ( >>= ) (x : ('a, 'b) t) (f : 'a -> ('c, 'b) t) : ('c, 'b) t = bind x f
...
module OptionMonad = struct
  ...
  let map f l =
    List.fold_right (fun x acc -> acc >>= fun acc -> f x >>| fun x -> x::acc) l (Some [])
...
module FilterMonad = struct
  ...
  let fold_left f acc l = 
    List.fold_left (fun acc x -> f acc x) acc l
