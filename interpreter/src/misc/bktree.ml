...
let rec create values = 
  ...
  | hd :: tl ->
    ...
    List.fold_left (fun acc value -> add levenshtein_distance acc value) tree tl
...
and add distance_fn tree value =
  ...
  | Node (v, children) ->
    ...
    if Hashtbl.mem children d then
      ...
      Hashtbl.add children d (create [value]);
    tree
...
and nearest_neighbor distance_fn tree query =
  let rec aux (best, best_dist) (Node (v, children)) =
    ...
    Hashtbl.fold (fun dist child (best, best_dist) ->
...
and levenshtein_distance a b = 
  ...
  let rec aux i j =
    if i = 0 then j
      ...
      min (min (aux (i-1) j + 1) (aux i (j-1) + 1)) (aux (i-1) (j-1) + cost)
  in
  aux (String.length a) (String.length b)

(* let () =
  let values = ["hello"; "hallo"; "hullo"; "hell"; "help"] in
  let tree = create values in
  let (nearest, dist) = nearest_neighbor levenshtein_distance tree "hell" in
  Printf.printf "Nearest neighbor: %s with distance: %d\n" nearest dist *)