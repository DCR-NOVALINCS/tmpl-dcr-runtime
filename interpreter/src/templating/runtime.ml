...
and update_event_value event expr_env =
  ...
  (match io.data with
  | Input ty ->
      eval_expr marking.data.value expr_env >>= fun value ->
      Ok (annotate ~loc:io.loc ~ty:!(io.ty) (Input ty), value)
  | Output expr ->
      eval_expr expr expr_env >>= fun value ->
...
and check_guard guard env =
  eval_expr guard env >>= fun guard_value ->
...
and propagate_effect relation event (event_env, expr_env) program =
  ...
  | SpawnRelation (from, guard, spawn_prog, _annot) ->
      if not (from.data = id.data) then Ok program
        ...
        check_guard guard expr_env >>= fun is_guard_true ->
        if not is_guard_true then Ok program
          ...
          Logger.debug
          @@ (List.map string_of_event inst_spawn_events |> String.concat "\n");
          Logger.debug "Instantiated relations from spawn:";
          Logger.debug
          @@ (List.map string_of_relation inst_spawn_relations
             |> String.concat "\n");
        ...
        if not is_guard_true then Ok program
              ...
              set_marking
                ~marking:
                  (mk_marking ~executed:dest_marking.data.executed.data
                     ~pending:true ~included:dest_marking.data.included.data
          ...
          | Exclude ->
              set_marking
                ~marking:
                  (mk_marking ~executed:dest_marking.data.executed.data
                     ~pending:dest_marking.data.pending.data ~included:false
                     ~value:dest_marking.data.value.data ())
                dest_event
