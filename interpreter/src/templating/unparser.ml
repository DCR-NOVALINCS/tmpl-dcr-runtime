...
module UnparserList = struct
  ...
  let unparse_list ?(initial = "") ?(separator = "") ?(buffer = Buffer.create 100) unparse_fn list = 
    match list with
    | [] -> ()
    | _ -> 
...
module PlainUnparser = struct 
  ...
  let rec unparse 
    ...
    >>= (print_template_decls, fun _ ->  
          ...
          ~buffer program.template_decls |> ignore;
    ...
    >>= (print_events || print_template_insts || print_relations, fun _ -> 
          ...
          ~print_events ~print_value:(print_events && should_print_value) ~print_executed:(print_events && should_print_executed_marking)
          ~print_template_insts ~print_relations 
  ...
  and unparse_template_decls 
    ...
    let unparse_template_decl ?(indent = "") ?(abbreviated = true) ?(buffer = Buffer.create 100) template_decl = 
      ...
      unparse_list ~buffer ~initial:": " ~separator:", " (fun ~buffer ty -> unparse_ty ~indent ~buffer ty |> ignore) export_types;
      (* Buffer.add_string buffer @@ " {\n"; *)
      ...
      unparse_list ~buffer ~initial:" => " ~separator:", " (fun ~buffer ex -> Buffer.add_string buffer @@ ex.data) export;
    in
    unparse_list ~buffer ~separator
    (fun ~buffer template_decl -> 
      unparse_template_decl ~indent ~abbreviated ~buffer template_decl) template_decls;
    Buffer.contents buffer
  ...
  and unparse_subprogram 
      ...
      ?(buffer = Buffer.create 100) (events, insts, relations) = 
    let open Misc.Monads.FilterMonad in
    return ()
    >>= (print_events, fun _ -> 
      unparse_events ~indent ~abbreviated 
      ~print_value ~print_executed
      ~buffer events |> ignore)    
  ...
  and unparse_events ?(indent = "") ?(abbreviated = true) ?(print_value = false) ?(print_executed = false) ?(buffer = Buffer.create 100) events = 
    ...
    let unparse_io ?(indent = "") ?(buffer = Buffer.create 100) io = 
      ...
      | Output expr -> 
          ...
          Buffer.add_string buffer @@ "]"
    in
    let unparse_marking ?(indent = "") ?(abbreviated = true) ?(buffer = Buffer.create 100) marking = 
      let unparse_marking_abbreviated 
        ...
        ()
        in
        let unparse_marking_extended ?(indent = "") ?(buffer = Buffer.create 100) marking = 
          Buffer.add_string buffer @@ indent;
      ...
      in
    let unparse_event ?(indent = "") ?(abbreviated = true) ?(buffer = Buffer.create 100) event = 
    ...
    unparse_list ~buffer ~separator:"\n"
    (fun ~buffer event -> 
      unparse_event ~indent ~abbreviated ~buffer event) events;
    Buffer.contents buffer

  and unparse_template_insts ?(indent = "") ?(buffer = Buffer.create 100) template_insts = 
    let unparse_inst ?(indent = "") ?(buffer = Buffer.create 100) inst = 
      ...
      Buffer.add_string buffer @@ "(";
      unparse_list ~buffer ~separator:", " (fun ~buffer (arg_name, expr) -> 
      ...
      Buffer.add_string buffer @@ ")";
      (* Buffer.add_string buffer @@ " => "; *)
      unparse_list ~buffer ~initial:" => " ~separator:", " (fun ~buffer x -> 
    ...
    unparse_list ~buffer ~separator:"\n"
    (fun ~buffer inst -> 
      unparse_inst ~indent ~buffer inst) template_insts;
    Buffer.contents buffer
  ...
  and unparse_relations ?(indent = "") ?(abbreviated = true) 
    ...
    let unparse_relation_arrow ~arrow_start ~guard:(guard_expr, guard_buffer) ?(buffer = Buffer.create 100) ~end_symbol _ =
    ...
    let unparse_relation_type ?(indent = "") ?(buffer = Buffer.create 100) ~guard relation_type = 
      Buffer.add_string buffer @@ indent;
      ...
      | Milestone -> "-><>"
      | _ -> "->" in
      unparse_relation_arrow ~arrow_start ~guard ~end_symbol:arrow_end ~buffer () |> ignore
    in
    let unparse_relation ?(indent = "") ?(abbreviated = true) ?(buffer = Buffer.create 100) relation =
      ...
      | SpawnRelation (from, guard, subprogram, annot) ->
        Buffer.add_string buffer @@ from.data;
        Buffer.add_string buffer @@ " ";
        unparse_relation_arrow ~arrow_start:"-" ~guard:(guard, guard_buffer) ~end_symbol:"->>" ~buffer () |> ignore;
        (* Buffer.add_string buffer @@ "-"; *)
        Buffer.add_string buffer @@ " {\n";
        let graph_indent = indent ^ "  " in
      ...
      unparse_list ~buffer ~separator:"\n"
      (fun ~buffer relation -> 
        unparse_relation ~indent ~abbreviated ~buffer relation) relations;
      Buffer.contents buffer

  and unparse_annotations ?(indent = "") ?(buffer = Buffer.create 100) annotations = 
    let unparse_annotation ?(indent = "") ?(buffer = Buffer.create 100) annotation = 
      ...
      | Foreach (x, l) -> 
        ...
        ()
      in
      unparse_list ~buffer ~initial:" - " ~separator:" | " (fun ~buffer annotation -> 
        unparse_annotation ~indent ~buffer annotation) annotations;
      Buffer.contents buffer
  ...
  and unparse_ty ?(indent = "") ?(buffer = Buffer.create 100) ty = 
    ...
    | RecordTy fields -> 
      Buffer.add_string buffer @@ "{ ";
      unparse_list ~buffer ~separator:", " (fun ~buffer (field, ty) -> 
  ...
  and unparse_expr ?(indent = "") ?(buffer = Buffer.create 100) expr = 
    ...
    | List es -> 
      Buffer.add_string buffer @@ "[ ";
      unparse_list ~buffer ~separator:", " (fun ~buffer e -> 
        unparse_expr ~indent ~buffer e |> ignore;
        ) es;
      Buffer.add_string buffer @@ " ]"
    | Record fields ->
      Buffer.add_string buffer @@ "{ ";
      unparse_list ~buffer ~separator:", " (fun ~buffer (field, e) -> 
        Buffer.add_string buffer @@ field.data;
        Buffer.add_string buffer @@ ": ";
        unparse_expr ~indent ~buffer e |> ignore; 
        ) fields;
