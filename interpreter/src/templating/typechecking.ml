...
let rec preprocess_subprogram ~ctxt ~events ~insts ~relations =
  ...
  >>| fun ctxt -> Logger.end_group () ; ctxt
...
and typecheck_subprogram ~events ~insts ~relations ~ctxt =
  ...
  >>= fun ctxt -> typecheck_relations ~ctxt relations

and typecheck_events ~ctxt events =
  let typecheck_event ctxt _event = Ok ctxt in
...
let typecheck ?ctxt program =
  ...
  let ctxt =
    ...
    | Some ctxt -> ctxt
    | None ->
        preprocess_subprogram ~ctxt:empty_context ~events:program.events
          ~insts:program.template_insts ~relations:program.relations
  in
...
let rec typecheck_expr ?(ty_env = empty_env) expr =
  ...
  let typecheck_list es ty_env =
    ...
    | None -> Ok (annotate @@ ListTy (annotate UnitTy))
    (* Empty List *)
  in
  let typecheck_record fields ty_env =
    map
      (fun (prop, e) ->
        typecheck_expr ~ty_env e >>| fun ty -> (annotate prop.data, ty) )
      fields
  ...
  | Trigger -> (
    match find_flat "@trigger" ty_env with
    | Some ty -> Ok ty
