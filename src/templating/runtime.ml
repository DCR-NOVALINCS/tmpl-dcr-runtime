...
and set_marking ?(marking = default_marking) event =
  ...
  Ok { event with data = { event.data with marking = { prev_marking with data = marking } } }
...
and propagate_effects event (event_env, expr_env) program =
  let relations = program.relations in
  fold_left_result
    (fun program relation -> propagate_effect relation event (event_env, expr_env) program)
    program relations
